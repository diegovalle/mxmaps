...
#' gg$render()
MXHexBinChoropleth <- R6Class("MXHexBinChoropleth", # nolint
  inherit = Choropleth2,
  public = list(
    ...
    render = function() {
      ...
      if (self$show_labels) {
        df_mxstate_labels <- structure(
          list(
            long = c(
              -107.424100471015, -107.424100471015,
...
#' @return A ggplot object representing an hexbin of Mexico
#' @examples
#' data(df_mxstate)
#' df_mxstate$value <- df_mxstate$pop
#' mxhexbin_choropleth(df_mxstate, num_colors = 1)
#' @export
mxhexbin_choropleth <- function(
  df, title = "",
  legend = "",
  # mxmaps extensions
