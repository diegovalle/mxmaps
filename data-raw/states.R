df_mxstate_2020 <- read.csv("data-raw/censo/2020.csv",
  fileEncoding = "windows-1252",
  skip = 3
)
names(df_mxstate_2020) <- c(
  "region", "state_name_official", "Total", "Male",
  "Female"
)
...
afro <- read.csv("data-raw/censo/afro.csv",
  fileEncoding = "windows-1252",
  skip = 3
)
names(afro) <- c(
  "region", "municipio_name", "Total", "afromexican",
  "nonafro", "NA", "em"
)
afro <- afro %>%
  filter(str_detect(region, "CONCATENAR")) %>%
...
ind <- read.csv("data-raw/censo/indi.csv",
  fileEncoding = "windows-1252",
  skip = 3
)
names(ind) <- c(
  "region", "municipio_name", "Total", "indigenous_language",
  "NonAfro", "NA", "em"
)
ind <- ind %>%
  filter(str_detect(region, "CONCATENAR")) %>%
...
df_mxstate_2020 <- df_mxstate_2020 %>%
  ...
  mutate(state_name_official = str_replace(
    state_name_official,
    "Distrito Federal",
    "Ciudad de México"
  )) %>%
  mutate(state_abbr_official = str_replace(
    state_abbr_official,
    "DF",
    "CDMX"
  ))
...
save(df_mxstate_2020,
  file = "data/df_mxstate_2020.RData",
