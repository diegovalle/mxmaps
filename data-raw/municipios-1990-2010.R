...
names(df_mxmunicipio_1990_2010) <- c(
  "region", "municipio_name", "sex",
  "pop1990",
...
df_mxmunicipio_1990_2010 <- df_mxmunicipio_1990_2010 %>%
  ...
  mutate(pop = as.integer(str_replace_all(pop, ",", "", fixed = TRUE))) %>%
  mutate(year = as.integer(str_replace_all(year, "pop", "", fixed = TRUE))) %>%
  mutate(sex = str_replace_all(sex,
    c("Hombre" = "Male", "Mujer" = "Female"),
    fixed = TRUE
  )) %>%
...
names(df_mxstate_1990_2010) <- c(
  "region", "municipio_name", "sex", "pop1990",
  "pop1995", "pop2000", "pop2005", "pop2010"
)
df_mxstate_1990_2010 <- df_mxstate_1990_2010[, 1:8]
df_mxstate_1990_2010 <- df_mxstate_1990_2010 %>%
  filter(str_detect(region, "CONCATENAR", fixed = TRUE)) %>%
  filter(municipio_name != "") %>%
